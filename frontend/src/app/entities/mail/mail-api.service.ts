...
export class MailAPI implements OnInit {
  ...
  setEmailFlagFlag(email_id : number, flag_value : boolean) {
    let the_token = localStorage.getItem("token");
    if (the_token == null) {
      the_token = "";
    }
    return this.http.put(`${API_URL}/api/${email_id}/flags/flagged`, {'flagged': flag_value.toString()}, {withCredentials: true, headers: new HttpHeaders({
      Authorization: `Bearer ${the_token}`
    })}).pipe();
  }
